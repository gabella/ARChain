...
!         IMPLICIT REAL*8 (A-H,M,O-Z) ! this is in 'ARCparams.CH'
     ...
     & ,cmxx(3),cmvx(3)
          LOGICAL NEWREG
          CHARACTER*40 outfile, outfileOld  !BG was *22 increased it.
...
c               This CONTANTS OF MOTION works only after first cal of ARC ! Therefore this is here
     ...
     & ,N ! print time, logH, N (of bodies left)
        call flush(6)
      endif  ! BG, is it faster without the stdout'put?
             ! Not obviously, took 12m00s for testSMBHplus.in.
...
200     continue
        ...
        vwr(j0+k)=v(i0+k)+cmvx(k) ! add centre-of-mass (remove cmxx if U need cm coor
        end do !k
        end do !i
...
C     BG 20191021 try to print out X's and V's with time.
        write(99,234)time,
     &       ((xwr(3*(ll-1)+jj), jj=1,3), (vwr(3*(ll-1)+kk), kk=1,3),
     &       ll=1,n_ini)
...
!-----------------------------------------------------
     ...
     & Terms(Ii,dX,dW,rij,rdotv,vij2,m(Ii),m(Jx),cl,DF,dfGR,spina,dsp)
         RS=2.d0*(m(i)+m(j))/CL**2
         ! BG 20190925 modify for wormhole, r_throat = 1.5 R_s,wh
         ! When is PN violated?
          test=1.2*RS ! Merge test set to 4*RS (OK?)
